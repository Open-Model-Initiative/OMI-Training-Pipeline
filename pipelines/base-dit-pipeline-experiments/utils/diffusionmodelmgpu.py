...
class MGPUDiffusionModelPipeline(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        model_to_load.diffusion_model.model.load_state_dict(dit_state_dict)
    ...
    def save_pretrained(self, save_directory):
        ...
        dit_state_dict = model_to_save.diffusion_model.model.state_dict()
        save_file(dit_state_dict, dit_save_path)
        
        ##If Ema model is not the same as the DiT, save the ema model
        if model_to_save.diffusion_model.emaModel is not model_to_save.diffusion_model.model:
            ema_save_path = os.path.join(save_directory, "ema_model.safetensors")
            ema_state_dict = model_to_save.diffusion_model.emaModel.state_dict()
    ...
    def train_step(self,
        ...
        for step, batch in enumerate(progress_bar):
            ...
            #pred
            pred_noise = self.diffusion_model.p(noisy_latents, timesteps, text_embed=text_embeds)
