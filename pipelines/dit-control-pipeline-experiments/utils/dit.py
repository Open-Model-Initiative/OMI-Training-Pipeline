...
class DiTBlock(torch.nn.Module):
    ...
    def __init__(self,channels,heads,conditionC, text_embed_dim, control_embed_dim):
        super().__init__()
        ...
        self.control_proj = torch.nn.Linear(control_embed_dim, channels)
    ...
    def forward(self,x,condition, text_embed, control_embed = None):
        # x: (B, C, H, W)
        ...
        if control_embed is not None:
            control_embed_proj = self.control_proj(control_embed)
        
        #concat embeddings
            context = torch.cat([text_embed_proj, control_embed_proj], dim = 1)
        else:
            context = text_embed_proj
...
class DiffusionTransformer(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks=torch.nn.ModuleList([
            DiTBlock(channels, nHeads, conditionC, text_embed_dim, control_embed_dim) 
    ...
    def forward(self, x, condition, text_embed, control_embed = None):
        ...
        for block in self.blocks:
            x = block(x, condition, text_embed, control_embed)
